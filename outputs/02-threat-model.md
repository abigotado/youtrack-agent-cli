...
| T30 | Concurrent apply and enrollment/rotation/revocation/recovery, multiple CLI/helper processes, helper restart, or active cleanup's read/delete ABA interval opens a gap between receipt validation and send | A stale key/context mutates, one receipt is sent twice, or cleanup deletes a replacement active lease | One launchd-managed helper owns a single serialized authority executor whose guard spans acquisition through close/read-delete cleanup; every registry commit and apply also contends on one helper-private fixed `active` Keychain account that survives restart; registry active binds a canonical pre-read intent and validates the candidate later under lease; recovery authenticates a new exact-code actor/session and performs fenced read-first cleanup; the dedicated ABA schedule queues acquisition between equality read/delete and proves zero competing Keychain work until guard release | Keychain/Security.framework or launchd/user-bootstrap compromise is outside the helper boundary; local denial of service remains possible |
| T31 | The embedded Developer ID profile expires after Gate or install but before confirmation/apply | Expired entitlement evidence continues authorizing Keychain/signing/write access | Descriptor binds exact `helper_profile_expires_at`; CMS semantic expiry must match; all 22 named Gate/grant/publication/install/runtime/apply/registry boundaries require trusted current time strictly before it; the closed 88-vector matrix proves just-before/equality/after/between-check behavior including peer authentication and every registry sign/commit fence. The only exception is exact launchd recovery-only launch/auth for status, trusted-UI fencing, journal CAS, close reconciliation, and guarded byte-equal cleanup—never ordinary authority | Clock/OS compromise is outside the local verifier boundary; expiry can intentionally deny availability |
| T32 | Activation smoke or post-grant verification inherits approval state, substitutes a registry after setup, or uses broad/repeated cleanup as a deletion primitive | Evidence appears to exercise the exact artifact while using stale/unbound authority, or cleanup deletes unrelated authority twice | Each capability plan begins with a token/context-limited first exact-artifact enrollment only after bounded canonical emptiness proofs for registry, coordinator, signing-key, journal, and mutable runner-session state; a retained revision-1/generation-1 SPKI/fingerprint/descriptor/session snapshot binds every later observation, index, and evidence set. Distinct root-token-bound `stage_cleanup` retains a complete attributed intent outside disposable state, requires coordinator active absent and exact byte equality, then publishes marker and cross-bound `delete_pending` progress through exclusive-`0600` no-follow temp writes, file/directory fsync, collision-safe no-replace publication, and reopen/hash verification before ACK and the sole delete. An unresolved marker permits only terminal absence reconciliation or quarantine, never re-delete | The disposable host/runner remains a trusted release input; partial/mismatched publication, present/unknown unresolved markers, and mismatched or expired cleanup delete nothing further, and the complete user/VM must be quarantined/destroyed with all output invalid |
