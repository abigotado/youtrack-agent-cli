...
| T31 | The embedded Developer ID profile expires after Gate or install but before confirmation/apply | Expired entitlement evidence continues authorizing Keychain/signing/write access | Descriptor binds exact `helper_profile_expires_at`; CMS semantic expiry must match; all 25 named Gate/grant/publication/install/runtime/apply/registry boundaries require trusted current time strictly before it; the closed 100-vector matrix proves just-before/equality/after/between-check behavior including peer authentication and every registry sign/commit fence. The only exception is exact launchd recovery-only launch/auth for status, trusted-UI fencing, journal CAS, close reconciliation, and guarded byte-equal cleanup—never ordinary authority | Clock/OS compromise is outside the local verifier boundary; expiry can intentionally deny availability |
