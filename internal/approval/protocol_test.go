...
func TestParseReceiptBytesRejectsNonCanonicalJSON(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "reordered fields", raw: bytes.Replace(valid, []byte(`{"schema_version":1,"receipt_id":"YTAR-AAAQEAYEAUDAOCAJBIFQYDIOB4"`), []byte(`{"receipt_id":"YTAR-AAAQEAYEAUDAOCAJBIFQYDIOB4","schema_version":1`), 1)},
